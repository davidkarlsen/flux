...
func (opts *controllerReleaseOpts) Command() *cobra.Command {
	...
	cmd.Flags().BoolVar(&opts.interactive, "interactive", false, "Interactive selection of updates to run")
...
func (opts *controllerReleaseOpts) RunE(cmd *cobra.Command, args []string) error {
	...
	if opts.interactive {
		...
		return interactive(cmd.OutOrStdout(), result)

	}
	return await(ctx, cmd.OutOrStdout(), cmd.OutOrStderr(), opts.API, jobID, !opts.dryRun, opts.verbosity)
}

func interactive(out io.Writer, result job.Result) error {
	menu := update.NewMenu(out, result.Result, 2)
	selected, aborted := menu.Run()
	if aborted {
		return nil
	}
	fmt.Println(selected, aborted)
	return nil
}
